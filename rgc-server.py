#Remote GPIO control server: version 2.0
import SocketServer
import sys
import hashlib
...
def planowanie(conndb2):
    cursor = conndb2.execute(
        "SELECT * from  planowanie p join stany s on p.Out_id = s.Id left join sensory se on se.Id = p.Warunek")
    for row in cursor:
        valueReached = False
        if row[1] == 'date':
            ...
            time = int(timelist[1]) - 1
            if time <= 0:
                valueReached = True
            else:
                conndb2.execute("UPDATE planowanie set Dane=?, Edit_time=? where Id=?", (str(
                    timelist[0])+','+str(time), datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f'), row[0]))
                conndb2.commit()
        else:
            if row[17] != None:
                sensorValue = getCurrentSensorValue(row[1],conndb2)
                valueReached = eval(str(sensorValue)+row[2]+row[4])
        if valueReached == True:
            if row[10] != row[6]:
                ...
                for gpio in gpiolist:
                    conndb2.execute("UPDATE stany set Stan =2,Edit_time=? where (GPIO_BCM like ? and Id!=? and IN_OUT like 'out') or (GPIO_BCM like ? and Id!=? and IN_OUT like 'out');", (
                        datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f'), "%"+gpio+",%", row[5], "%,"+gpio+"%", row[5]))
                    conndb2.execute("UPDATE stany set Stan =?,Edit_time=? where GPIO_BCM =? and Id!=? and IN_OUT like 'out' ;", (
                        set, datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f'), gpio, row[5]))
                conndb2.execute("UPDATE stany set Stan =?, Edit_time=? where Id=?", (
                    set, datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f'), row[5]))
                conndb2.execute("INSERT INTO historia(Typ, Id_IO, Stan) VALUES(?,?,?)", ("scheduled", row[5], "ON" if (
                    (set and not row[14]) or (not set and row[14])) else "OFF"))
            if row[3] == 'once' or row[1] == 'date':
                conndb2.execute(
                    "DELETE from planowanie where Id=?", (row[0],))
                conndb2.execute("UPDATE planowanie set Edit_time=? where Id in (SELECT Id FROM planowanie LIMIT 1)", (
                    datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f'),))
            elif row[1] == 'timer':
                conndb2.execute("UPDATE planowanie set Dane=?, Edit_time=? where Id=?", (
                    timelist[0]+','+timelist[0], datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f'), row[0]))
            conndb2.commit()
...
def actions(conndb4,updateSignal,idCE = 0,changedBy="scheduled"):
    ...
    for row in cursor:
        ...
        for rowW in cursorW:
            if rowW[4] == 'date':
                conditions.append(eval("datetime.strptime('"+rowW[6]+"','%Y-%m-%d %H:%M')"+rowW[5]+"datetime.utcnow()"))
            elif rowW[4] == 'hour':
                ...
                conditions.append(eval(rowW[6]+rowW[5]+"datetime.utcnow().weekday()"))
            elif rowW[4] == 'i/o':
                planned = int(rowW[6])
                cState = int(rowW[18])
                reverse = int(rowW[22])
    ...
    def handle(self):
        ...
        if passwalidation == True:
            if datalist[1] == 'version_check':
                ...
                r1 = conndb.execute(
                    "DELETE from planowanie where Out_id=?", (datalist[2],)).rowcount
                if r1 > 0:
                    conndb.execute("UPDATE planowanie set Edit_time=? where Id in (SELECT Id FROM planowanie LIMIT 1)", (
                        datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f'),))
                pwmpins = datalist[3].split(",")
                for pin in pwmpins:
                ...
                break_ = int(datalist[2])
                conndb.execute(
                    "DELETE from pwm where Id=?", (datalist[2],))
                conndb.execute(
                    "DELETE from historia where Id_Pwm=?", (datalist[2],))
                ...
                cursor = conndb.execute(
                    "SELECT Max(Edit_time) FROM planowanie")
                for row in cursor:
                ...
                for row in cursor:
                    reply += str(row[0])+';'+str(row[1])+';'+str(row[2])+';'+str(row[3])+';'+str(
                        row[4])+';'+str(row[6])+';'+str(row[11])+';'+str(row[14])+';'+str(row[18])+';'+str(row[22])+';'
                ...
                conndb.execute("INSERT INTO planowanie(Warunek, Podwarunek, Rodzaj, Dane, Out_id, Stan, Edit_time) VALUES(?,?,?,?,?,?,?)", (
                    datalist[2], datalist[3], datalist[4], datalist[5], datalist[6], datalist[7], datalist[8]))
                reply = 'true;Insert_Action;'
            elif datalist[1] == 'Update_Action':
                conndb.execute("UPDATE planowanie set Warunek=?, Podwarunek=?, Rodzaj=?, Dane=?, Out_id=?, Stan=?, Edit_time=? where Id=?", (
                    datalist[2], datalist[3], datalist[4], datalist[5], datalist[6], datalist[7], datalist[9], datalist[8]))
                reply = 'true;Update_Action;'
            elif datalist[1] == 'Delete_Action':
                conndb.execute(
                    "DELETE from planowanie where Id=?", (datalist[2],))
                conndb.execute("UPDATE planowanie set Edit_time=? where Id in (SELECT Id FROM planowanie LIMIT 1)", (
                    datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f'),))
                ...
                for row in cursor:
                    reply += str(row[0])+";"+str(row[1])+";"+row[4]+";"+str(row[9])+";"+str(row[2])+";"+str(row[3])+";"+str(row[5])+";"+row[10]+";"
                ...
                conndb.execute(
                    "DELETE from sensory where Id=?", (datalist[2],))
    ...
    def handle(self):
        ...
        if PASSWORD == '':
            ...
            if datalist[0] == PASSWORD:
                ...
                passwalidation = False
        if debug: print 'RECIVED: '+data
        if debug: print 'SIZE: '+str(sys.getsizeof(data))+"b"
        if passwalidation == True and len(datalist) > 1:
            if datalist[1] == 'version_check':
                reply = 'true;version_check;'+str(CODE_VERSION)+';'
            elif datalist[1] == 'Server_logs':
                logs = os.popen('journalctl -u rgc.service -n 200 --no-pager').read()
                ...
                reply += datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')+';'+str(getCurrentSensorValue(datalist[2],conndb))+";"
            ...
            reply = 'false;Wrong password or wrong request!;'
        if debug: print "REPLY: "+reply
        if PASSWORD != '' and passwalidation == True:
            reply = '1;'+encrypt(ENC_KEY, reply)+';'
...
if __name__ == '__main__':
    ...
    log.info('Server time: '+datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f'))
